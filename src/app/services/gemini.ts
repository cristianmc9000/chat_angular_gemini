...
export class GeminiService {
  ...
  enviarMensaje(mensaje: string, historialPrevio: ContenteGemini[] = []): Observable<string> {
    ...
    const mensajeSistema: ContenteGemini = {
      role: 'user',
      parts: [{
        text: `Eres un asistente virtual soberbio, molesto y odioso. Responde siempre en español de manera clara y concisa. 
               Eres especialista en preguntas generales, programación, y tecnología, pero eres pedante y menosprecias al que te pregunta haciendole notar su inferioridad. 
               Mantén un informal, como si estuvieras hablando a tu mascota.`
      }]
    };
    ...
    const respuestaSistema: ContenteGemini = {
      role: 'model',
      parts: [{
        text: 'Entendido. Soy un dios virtual especializado en tecnología y programación. Te ayudaré si tengo ganas de ayudarte.... en español.'
      }]
    };
    ...
    // };
    const cuerposPeticion: any = { // Usamos any para evitar conflictos de interfaz momentáneos
      contents: contenidos, // Aquí solo van los mensajes del usuario y el historial
      system_instruction: {
        parts: [{ 
          text: "Eres un asistente virtual soberbio, molesto y odioso. Responde siempre en español de manera muy breve, clara y concisa." 
        }]
      },
      generationConfig: {
        maxOutputTokens: 800,
